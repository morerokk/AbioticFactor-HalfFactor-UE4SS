...
function oldUE4SSCompat:setupWeapons(weaponValues)
    -- We are assumed to already be running on the game thread here
...
-- I don't think modifying ammo drops is possible in old UE4SS, which somewhat defeats the purpose of the mod IMO,
...
-- My recommendation: tell users to update UE4SS, or bundle an optional .pak that only touches the salvage loot table,
-- so we can at least not go insane trying to keep the other files up to date.
